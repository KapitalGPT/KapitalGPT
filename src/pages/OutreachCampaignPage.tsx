                  ...
                  <div key={investor.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center space-x-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          investor.matchScore >= 90 ? 'text-green-600 bg-green-100' :
                          investor.matchScore >= 80 ? 'text-blue-600 bg-blue-100' :
                          investor.matchScore >= 70 ? 'text-yellow-600 bg-yellow-100' :
                          'text-gray-600 bg-gray-100'
                        }`}>
                          {investor.source === 'manual' ? 'Manual' : `${investor.matchScore}% Match`}
                        </span>
                      ...
                      >
                        <Star className={`w-5 h-5 ${investor.isFavorite ? 'text-yellow-500 fill-current' : 'text-gray-400'}`} />
                      </button>
                    </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-2">{investor.type === 'vc' ? 'Venture Capital' : investor.type}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                          ...
                          <div>
                            ...
                            <p className="font-medium">{Array.isArray(investor.focus) ? investor.focus.slice(0, 2).join(', ') : investor.focus}</p>
                          </div>
                        </div>
                          ...
                          <div className="flex space-x-2">
                            <button 
                              onClick={() => {
                                ...
                                const newTextsReplied = textsRepliedCount + Math.floor(Math.random() * 2);
                                ...
                                updateInvestorStatus(investor.id, 'emailed');
                              }}
                            ...
                            <button 
                              onClick={() => {
                                const newCallsCount = callsMadeCount + 1;
                                const newCallsAnswered = callsAnsweredCount + Math.floor(Math.random() * 2);
                                setCallsMadeCount(newCallsCount);
                                setCallsAnsweredCount(newCallsAnswered);
                                localStorage.setItem('crm_calls_made', newCallsCount.toString());
                                localStorage.setItem('crm_calls_answered', newCallsAnswered.toString());
                                updateInvestorStatus(investor.id, 'follow_up_needed');
                              }}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-right">
                  <div className="text-xl font-bold text-purple-600">{investors.filter(inv => inv.isFavorite).length}</div>
                  <div className="text-xs text-gray-600">Favorites</div>
                </div>
              </div>

              {investors.filter(inv => inv.isFavorite).length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <div className="flex items-center justify-center space-x-1 text-purple-600">
                    ...
                    <span className="text-sm">Click the star icon to add favorites</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {investors.filter(inv => inv.isFavorite).map(investor => (
                    <div key={investor.id} className="border border-purple-200 rounded-lg p-3 bg-purple-50">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-600">{investor.type === 'vc' ? 'Venture Capital' : investor.type}</p>
                        </div>
                        <button
                          onClick={() => {
                            setInvestors(prev => prev.map(inv => 
                              inv.id === investor.id 
                                ? { ...inv, isFavorite: false }
                                : inv
                            ));
                          }}
                      ...
                      <div className="flex items-center justify-between">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          investor.source === 'manual' ? 'bg-purple-100 text-purple-600' :
                          investor.matchScore >= 90 ? 'text-green-600 bg-green-100' :
                          investor.matchScore >= 80 ? 'text-blue-600 bg-blue-100' :
                          investor.matchScore >= 70 ? 'text-yellow-600 bg-yellow-100' :
                          'text-gray-600 bg-gray-100'
